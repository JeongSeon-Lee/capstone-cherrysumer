...
class UploadSuccessFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        // 레이아웃을 인플레이트
        val binding = FragmentPostWriteUploadBinding.inflate(inflater, container, false)
        ...
        // Complete 버튼 클릭 시 PostWriteFragment로 이동
        binding.completeButton.setOnClickListener {
            // PostWriteFragment로 이동 (스택에 추가하지 않음)
            val transaction = parentFragmentManager.beginTransaction()
            transaction.replace(R.id.fragment_container, PostWriteFragment())
